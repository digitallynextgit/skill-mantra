        ...
        <section className=" flex justify-center pt-[100px] md:flex-row flex-col items-center px-[5vw] md:py-[5vw] py-[10vw] gap-32 bg-blue-90 overflow-hidden">
            ...
            <div className="">
                ...
                <div className="flex flex-col z-20">
                    {data3.sections.map((section, index) => (
                        <div key={index} className="flex items-center mb-4">
                            <Image src="/icons/tick.webp" alt="icon" width={84} height={84} className="w-[64px] h-[64px]" />
                            <span className="text-xl md:text-4xl  font-abril ml-4 font-semibold text-white">{section.title}</span>
                        </div>
                    ))}
                    <div>
                        <Button asChild className="bg-[#FFC224] py-8 px-8 text-xl rounded-xl text-blue-90 flex justify-center items-center lg:w-[40%]">
                            <Link href="https://courses.skillmantra.in/login" className='text-center'>Enroll for a Free Demo to <br/> Supercharge Your Career!</Link>
                        </Button>
                    </div>
                </div>
                <div className="mt-8">
                    {data3.subsections.map((subsection, index) => (
                        <div key={index} className="flex items-center mb-2 justify-center lg:justify-start lg:items-start z-20">
                            <span className="text-lg  text-white font-semibold">{subsection.title}</span>
                        </div>
                    ))}
                    <motion.img
                        src="/icons/dots.webp"
